...
const T: Record<Lang, any> = {
  en: {
    ...
    history: "Meeting history", cal: "📅 Open meeting calendar →", empty: "No saved meetings on this device yet.", download: "Download", del: "Delete", deviceNote: "History is stored on this device.",
  },
  vi: {
    ...
    history: "Lịch sử cuộc họp", cal: "📅 Mở Lịch họp →", empty: "Chưa có cuộc họp nào lưu trên thiết bị này.", download: "Tải về", del: "Xoá", deviceNote: "Lịch sử được lưu trên thiết bị này.",
  },
  ko: {
    ...
    history: "회의 기록", cal: "📅 회의 캘린더 열기 →", empty: "이 기기에 저장된 회의가 없습니다.", download: "다운로드", del: "삭제", deviceNote: "기록은 이 기기에 저장됩니다.",
  },
};
...
const S: Record<string, React.CSSProperties> = {
  wrap: { minHeight: "100vh", maxWidth: 720, margin: "0 auto", padding: "clamp(14px,4vw,24px) clamp(14px,4vw,20px) 60px", fontFamily: FONT, color: "#0a1124" },
  ...
  h1: { fontSize: "clamp(22px,6vw,28px)", fontWeight: 900, letterSpacing: "-.03em", marginBottom: 3 },
  subEmail: { fontSize: "clamp(12.5px,3.4vw,13.5px)", color: "#7b88a3", fontWeight: 600, marginBottom: "clamp(14px,4vw,22px)" },
  card: { background: "#fff", border: "1px solid #e7ebf3", borderRadius: "clamp(13px,3.6vw,16px)", padding: "clamp(14px,3.8vw,18px) clamp(14px,4.2vw,20px)", marginBottom: "clamp(10px,3vw,14px)", boxShadow: "0 12px 30px -26px rgba(10,17,36,.5)" },
  ...
  planName: { fontSize: "clamp(19px,5.2vw,22px)", fontWeight: 900, letterSpacing: "-.02em", marginTop: 3 },
  btnGhost: { fontSize: "clamp(12.5px,3.4vw,13px)", fontWeight: 800, color: "#0a1124", background: "#fff", border: "1.5px solid #e3e8f2", borderRadius: 10, padding: "clamp(7px,2vw,9px) clamp(12px,3.4vw,15px)", textDecoration: "none" },
  btnPri: { fontSize: "clamp(12.5px,3.4vw,13px)", fontWeight: 800, color: "#fff", background: "#1f6bff", border: "1.5px solid transparent", borderRadius: 10, padding: "clamp(7px,2vw,9px) clamp(12px,3.4vw,15px)", textDecoration: "none" },
