...
const C: Record<Lang, LegalContent> = {
  en: {
    ...
    meta: `Last updated: ${UPDATED} · Applies to **meet.transflash.app** (“Flash Meet”).`,
    sections: [
      { h: "1. Acceptance", blocks: [{ p: "By accessing or using Flash Meet, you agree to these terms. If you do not agree, please do not use the service." }] },
      { h: "2. Service description", blocks: [{ p: "Flash Meet provides transcription, multi-language live translation, speaker separation and AI meeting summaries. The service may change, be upgraded, or have features paused over time." }] },
      ...
      { h: "8. Limitation of liability", blocks: [{ p: "To the extent permitted by law, Flash Meet is not liable for indirect, incidental or consequential damages arising from use of the service." }] },
  ...
  vi: {
    ...
    meta: `Cập nhật lần cuối: ${UPDATED} · Áp dụng cho **meet.transflash.app** (“Flash Meet”).`,
    sections: [
      { h: "1. Chấp nhận điều khoản", blocks: [{ p: "Khi truy cập hoặc sử dụng Flash Meet, bạn đồng ý với các điều khoản này. Nếu không đồng ý, vui lòng không sử dụng dịch vụ." }] },
      { h: "2. Mô tả dịch vụ", blocks: [{ p: "Flash Meet cung cấp phiên âm, dịch trực tiếp đa ngôn ngữ, tách giọng nói và tóm tắt cuộc họp bằng AI. Dịch vụ có thể thay đổi, nâng cấp hoặc tạm ngừng tính năng theo thời gian." }] },
      ...
      { h: "8. Giới hạn trách nhiệm", blocks: [{ p: "Trong phạm vi pháp luật cho phép, Flash Meet không chịu trách nhiệm cho các thiệt hại gián tiếp, ngẫu nhiên hoặc hệ quả phát sinh từ việc sử dụng dịch vụ." }] },
  ...
  ko: {
    ...
    meta: `최종 업데이트: ${UPDATED} · 적용 대상 **meet.transflash.app** (“Flash Meet”).`,
    sections: [
      { h: "1. 약관 동의", blocks: [{ p: "Flash Meet에 접속하거나 이용함으로써 본 약관에 동의하게 됩니다. 동의하지 않으면 서비스를 이용하지 마십시오." }] },
      { h: "2. 서비스 설명", blocks: [{ p: "Flash Meet는 전사, 다국어 실시간 번역, 화자 분리, AI 회의 요약을 제공합니다. 서비스는 시간이 지남에 따라 변경·개선되거나 기능이 중단될 수 있습니다." }] },
      ...
      { h: "8. 책임의 제한", blocks: [{ p: "법이 허용하는 범위 내에서 Flash Meet는 서비스 이용으로 인한 간접적·부수적·결과적 손해에 대해 책임지지 않습니다." }] },
